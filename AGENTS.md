...
- **Known pre-existing issues on the current checkout** (not caused by env setup, do not block setup): `npm run lint` (Biome) reports a formatting error, `npm run typecheck` reports several type errors, and one test - `tests/web-server.test.ts > rejects oversized payloads with 413` - times out. The remaining 629 tests pass.
