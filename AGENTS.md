...
- **To exercise the engine end-to-end without any agent**, use an "agentless" workflow whose steps are only `distributor`/`consolidator`/`gate` (and optionally `llm` steps with an API key in the environment). Define it in a `steamtrain.json` and run via `--config-file <path>`; the agent binary preflight is skipped when a workflow spawns no agents, though llm steps still require their provider API key (e.g. `ANTHROPIC_API_KEY`).
